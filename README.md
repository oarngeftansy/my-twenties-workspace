...
GitHub Pages 现在是云端工作台的固定入口。云端地址：https://my-twenties-studio.neat-snow-9462.chatgpt.site 。使用创建工作台的 ChatGPT 账号登录。

- 记录、Kimi K3 对话和内容审查保存在云端，在不同设备登录同一账号可继续写。并发编辑通过版本检查防止覆盖。
...
- Kimi 密钥仅配置在云端服务环境，不包含在 GitHub 页面或仓库。旧版本公开资料的授权保持不变。
...
GitHub Pages 构建只包含 `pages/main.tsx` 引用的入口与迁移代码，不部署 D1 或服务器凭据。云端源码位于 `cloud-workspace/`，独立构建并通过 Sites 发布；根目录旧版工作台源码保留供查阅。
